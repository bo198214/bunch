...
main(){ FILE *o; o=fopen("figLb.eps","w"); ado(o,230,100);
...
fprintf(o,"1 setlinecap 1 setlinejoin .5 W  0 0 1 RGB S\n");

M(1/M_E,M_E)
for(n=2;n<170;n+=2) {x=1./M_E+0.01*n;  z_type Lb;  Lb=lob(x);
 	printf("lob(%6.3f)=  %6.3f +i* %6.3f\n",x,Re(Lb),Im(Lb) );
	 y=Re(Lb); if(n/4*4==n)  M(x,y) else L(x,y) 	}
fprintf(o,".6 W 0 0 1 RGB S\n");

fprintf(o,"1 0 0 RGB S\n");
o(1./M_E,0.)
for(n=1;n<90;n+=2) {x=1./M_E+0.001*n*n/(1+.04*n);  z_type Lb;  Lb=lob(x);
 	printf("lob(%6.3f)=  %6.3f +i* %6.3f\n",x,Re(Lb),Im(Lb) );
	 y=Im(Lb);  o(x,y) 	}
for(n=1;n<90;n+=2) {x=1./M_E+0.001*n*n/(1+.04*n);  z_type Lb;  Lb=lob(x);
 	printf("lob(%6.3f)=  %6.3f +i* %6.3f\n",x,Re(Lb),Im(Lb) );
...
for(n=0;n<10;n++){x=.1/M_E*n;o(x,0)}
...
fprintf(o,"%cTrailer\n",'%');
fclose(o);
//	system(    "open figLb.eps");// for macintosh
	system("epstopdf figLb.eps");
	system(    "xpdf figLb.pdf");// for linux
